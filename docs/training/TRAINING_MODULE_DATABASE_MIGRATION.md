...
> **File to create:** `supabase/migrations/YYYYMMDDHHMMSS_training_module.sql`
> 
> Replace YYYYMMDDHHMMSS with current timestamp (e.g., `20251210150000`)
...
(
  ...
  '{
    ...
    "fields": ["employee_name", "content_title", "status", "due_date", "completed_at"],
    "filters": [],
...
(
  ...
  '{
    "data_source": "reporting.training_status",
    "fields": ["employee_name", "employee_email", "content_title", "due_date", "days_remaining"],
...
(
  ...
  '{
    "data_source": "reporting.training_status",
    "fields": ["employee_name", "content_title", "status", "completed_at"],
...
-- Check enums exist
SELECT typname FROM pg_type WHERE typname LIKE 'training%';

-- Check view exists
SELECT table_name FROM information_schema.views 
WHERE table_schema = 'reporting' AND table_name = 'training_status';
...
SELECT id, name, public FROM storage.buckets WHERE id = 'training-content';

-- Check report templates
SELECT name, category FROM saved_reports WHERE category = 'Training';
```
...
DROP VIEW IF EXISTS reporting.training_status;
DROP TABLE IF EXISTS public.training_notifications;
DROP TABLE IF EXISTS public.training_completions;
DROP TABLE IF EXISTS public.training_assignments;
DROP TABLE IF EXISTS public.training_content;
DROP TYPE IF EXISTS training_status;
DROP TYPE IF EXISTS training_content_type;

-- Remove storage bucket
DELETE FROM storage.buckets WHERE id = 'training-content';

-- Remove report templates
DELETE FROM saved_reports WHERE category = 'Training' AND is_template = true;
```
