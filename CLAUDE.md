...
56. **Refresh-token cascade has a SECOND, deeper cause — supabase-js `__loadSession` keeps deciding the just-refreshed token is "in expiry margin" (May 12, 2026)** — Confirmed live during diagnosis: even after PRs #65/#66/#67/#69 + Gotcha #55's MobileTimeTracker fix, an admin login on `/` still produces 30+ `/auth/v1/token?grant_type=refresh_token` POSTs at ~95–130 ms intervals (auth audit log + browser network trace). Auth audit log shows the refresh:login ratio jumped from 1–3× (May 4 and earlier) to **7–17×** on May 7 onward and has stayed there — today it's 15 logins → 259 refreshes. PR #66's `autoRefreshToken: false` correctly disables the background ticker and `_recoverAndRefresh` path, but does NOT disable the *in-margin* refresh inside `__loadSession` (auth-js v2.71.1, [GoTrueClient.js:1050](node_modules/@supabase/auth-js/dist/main/GoTrueClient.js) — `if (hasExpired)` where `hasExpired = expires_at*1000 - Date.now() < EXPIRY_MARGIN_MS (90s)`). Every Supabase data API call (`from()`, `functions.invoke`, realtime subscribes) routes through `SupabaseClient._getAccessToken → auth.getSession() → __loadSession`, so on the post-login burst, dozens of serial `_callRefreshToken` calls fire. Each succeeds, the server issues a NEW token, and the very next `__loadSession` *still* reads it back as "in margin" and refreshes again. Continues until Supabase's 30-per-5-min refresh-token rate limit 429s → supabase-js `_removeSession()` → broadcast `SIGNED_OUT` → AppLayout bounce.
    ...
    **Why each fresh token reads as in-margin is still unverified.** Three remaining hypotheses: (a) GoTrue's `JWT_EXP` for this project was lowered to < 90 s in the Supabase dashboard (Authentication → Sessions → JWT Expiry) some time around May 6–7 — postgres's `app.settings.jwt_exp` shows 3600 but that's PostgREST's setting, not GoTrue's; (b) clock skew between server and client; (c) a library bug in `_saveSession`'s `expires_at` computation. The 1.2× → 17× jump on May 7 strongly points to a config change, but at the time of writing the dashboard hasn't been spot-checked.

    **The fix shipped is a client-side `_callRefreshToken` rate-limit in [src/integrations/supabase/client.ts](src/integrations/supabase/client.ts)**: if the previous refresh succeeded < 30 s ago, the next call short-circuits to the cached result instead of contacting the server. Kills the cascade regardless of which root-cause hypothesis is correct. Safe because the server-issued token is valid for at least its issued TTL (≥60 s on any sane Supabase config), and real refresh-after-expiry still works after the cooldown.
    ...
    **Followup the next session should do**: (1) verify in Supabase dashboard whether GoTrue `JWT_EXP` was changed — if so, restore to 3600 s; with that restored, the rate-limit in client.ts becomes pure belt-and-suspenders rather than load-bearing; (2) once the root cause is verified, simplify the patch — possibly remove it if (a) is true and the dashboard is corrected; (3) the audit-log SQL above is also a good hourly cron alert candidate for catching future regressions in the same class.
