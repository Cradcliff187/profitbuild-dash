...
56. **Refresh-token cascade has a SECOND, deeper cause — supabase-js `__loadSession` keeps deciding the just-refreshed token is "in expiry margin" (May 12, 2026)** — Confirmed live during diagnosis: even after PRs #65/#66/#67/#69 + Gotcha #55's MobileTimeTracker fix, an admin login on `/` still produces 30+ `/auth/v1/token?grant_type=refresh_token` POSTs at ~95–130 ms intervals (auth audit log + browser network trace). Auth audit log shows the refresh:login ratio jumped from 1–3× (May 4 and earlier) to **7–17×** on May 7 onward and has stayed there — today it's 15 logins → 259 refreshes. PR #66's `autoRefreshToken: false` correctly disables the background ticker and `_recoverAndRefresh` path, but does NOT disable the *in-margin* refresh inside `__loadSession` (auth-js v2.71.1, [GoTrueClient.js:1050](node_modules/@supabase/auth-js/dist/main/GoTrueClient.js) — `if (hasExpired)` where `hasExpired = expires_at*1000 - Date.now() < EXPIRY_MARGIN_MS (90s)`). Every Supabase data API call (`from()`, `functions.invoke`, realtime subscribes) routes through `SupabaseClient._getAccessToken → auth.getSession() → __loadSession`, so on the post-login burst, dozens of serial `_callRefreshToken` calls fire. Each succeeds, the server issues a NEW token, and the very next `__loadSession` *still* reads it back as "in margin" and refreshes again. Continues until Supabase's 30-per-5-min refresh-token rate limit 429s → supabase-js `_removeSession()` → broadcast `SIGNED_OUT` → AppLayout bounce.
    ...
    **Root cause confirmed live (May 12 PR #79 diagnosis): hypothesis (a) — GoTrue's `JWT_EXP` is set well below 90 s in the Supabase dashboard for this project.** Captured in a real login during PR #79 verification: 30 s after issue, the stored session's `expires_at` was **54 s in the past** (`expires_in_seconds_from_now: -54`). The only way that's possible is if the server-issued access token's TTL is < ~90 s — almost certainly **60 s** (a common "security hardening" misconfig). Postgres `app.settings.jwt_exp = 3600` is PostgREST's setting, not GoTrue's, so it didn't tell us this. The 1.2× → 17× refresh:login jump on May 7 in the audit log lines up with when the GoTrue config was likely changed.

    **Two-layer fix:**
    1. **Code (PR [#79](https://github.com/Cradcliff187/profitbuild-dash/pull/79), shipped May 12):** client-side `_callRefreshToken` rate-limit in [src/integrations/supabase/client.ts](src/integrations/supabase/client.ts) — if the previous refresh succeeded < 30 s ago, the next call short-circuits to the cached result instead of contacting the server. Kills the cascade regardless of server-side TTL. This is the load-bearing fix until layer 2 is done.
    2. **Server config (manual dashboard action, separate from the PR):** in [Supabase Dashboard → Authentication → Sessions](https://supabase.com/dashboard/project/clsjdxwbsjbhjibvlqbz/auth/sessions), set **Access token expiry time** to `3600` (1 hour, the Supabase default). With this restored, the client-side rate-limit becomes pure belt-and-suspenders — defense in depth, no longer load-bearing.
    ...
    **Followup (one task remaining, low-urgency since cascade is dead):** restore Access token expiry in the Supabase dashboard to 3600 s (link above). The rate-limit code is intentionally kept after that's done — it costs nothing and protects against future TTL misconfigs or library regressions. **Don't remove it** without first verifying the audit-log diagnostic SQL shows zero refreshes-within-2-seconds across a 24-hour window. The audit-log SQL above is also a good hourly cron alert candidate for catching future regressions in this same class.
...
| Issue | Resolution |
|-------|-----------|
| **P0 login loop reactivated for ALL users (admin + field worker) after recent updates shipped (PR [#79](https://github.com/Cradcliff187/profitbuild-dash/pull/79), May 12 2026)** | Fixed — see new Gotchas #55 (per-role landing-page gap) and #56 (server-side TTL too short for supabase-js's 90 s expiry margin). Two compounding causes: (1) `MobileTimeTracker` on the field-worker post-login landing `/time-tracker` still had two `postgres_changes` realtime channels + a mount-time `supabase.auth.getUser()` — PR #67 missed it because it only covered the admin landing `/`; (2) verified live via in-browser inspection that GoTrue's `JWT_EXP` on this project is set far below 90 s (session `expires_at` was 54 s in the past, 30 s after issue) — every fresh token reads as "in margin" inside `__loadSession`, triggering an infinite refresh loop. **Fix shape**: client-side `_callRefreshToken` rate-limit kills the cascade regardless of server TTL; MobileTimeTracker realtime/getUser removed mirrors PR #67. Verified against all 3 user tiers (chris.l.radcliff@gmail.com, chris@radcliffcg.com admin; TomFinn@myyahoo.com field worker) — each login produces 1 password grant + 1 refresh, lands on correct route, stays stable 25+ s. Audit log: today's broken baseline of 15 logins → 259 refreshes (17×) collapsed to 3 logins → 5 refreshes (1.7×). **Outstanding manual followup** (low-urgency, cascade is dead): set Supabase Dashboard → Authentication → Sessions → Access token expiry to 3600 s; rate-limit then becomes belt-and-suspenders. |
