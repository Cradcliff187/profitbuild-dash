...
| Audit 7 project-route files for direct Supabase writes that bypass invalidation | `src/components/project-routes/*.tsx` | Companion to the `useProjectData` TanStack migration (Apr 18, 2026). The hook now invalidates on `loadProjectData()` calls, but if a route file writes directly via `supabase.from(...).update(...)` in its own handler and never calls `loadProjectData()` on success, the Project Detail View will still appear stale. Each `onSuccess` / `onSave` handler in the 7 route files needs a quick scan to confirm it either (a) calls `loadProjectData()` from outlet context, or (b) invokes `queryClient.invalidateQueries({ queryKey: ['project-data', projectId] })` directly. |
