...
### 33. Time-Approval Definition is one DB column — `expenses.is_time_entry` (Jun 3, 2026, PR #143)

"Is this `expenses` row a labor/time record vs. a money expense?" is answered by ONE
**GENERATED STORED** column — the single source of truth for the entire time-approval domain:

```sql
expenses.is_time_entry := (category = 'labor_internal' OR start_time IS NOT NULL)
```

- `category = 'labor_internal'` → all internal labor **including every PTO type** (Sick/Vacation/
  Holiday on `006-SICK`/`007-VAC`/`008-HOL`), guaranteed by the Rule 6a category lock **regardless of
  hours or clock times** (PTO is keyed as hours with NULL start/end; some historical PTO even has NULL
  hours — so an `hours > 0` discriminator was explicitly rejected).
- `start_time IS NOT NULL` → any clocked entry, **including subcontractor time** (category
  `subcontractors`).
- Money expenses (materials, gas, the subcontractor *bills*, etc.) have neither → `false`.
- GENERATED STORED ⇒ self-populating for all existing + future rows: **no backfill, no trigger**, can't
  be set wrong by any code path. Mirrors the `labor_cushion_amount` generated-column pattern (Gotcha #62).

**Why it exists:** three surfaces each re-implemented the discriminator and drifted into three
different answers — the table required `start_time AND end_time` (missed all PTO), the sidebar badge
used `start_time NOT NULL` (missed all PTO), and the page-tab badge used `category='labor_internal'`
(missed subcontractor time). Net effect: PTO was *counted* by a badge but the table could never
*display* it (no clock times), so the count was stuck with no rows to act on and no in-app way to
approve. See Gotcha #68.

**The single source of truth lives in [src/utils/timeEntries.ts](src/utils/timeEntries.ts)** —
`PTO_PROJECT_NUMBERS` + `isPTOProject()` (previously hardcoded in **6** files), plus the shared
PostgREST clauses `EXCLUDE_ACTIVE_TIMERS_OR` (`end_time.not.is.null,start_time.is.null` — keeps
completed clocked entries + clockless PTO, drops in-progress timers) and `PENDING_APPROVAL_OR`. The
three surfaces consume them:

| Surface | File | Filter |
|---|---|---|
| Approval **table** | [useTimeEntries.ts](src/hooks/useTimeEntries.ts) | `.eq('is_time_entry', true).or(EXCLUDE_ACTIVE_TIMERS_OR)` + user filters |
| **Sidebar** badge | [usePendingCounts.ts](src/hooks/usePendingCounts.ts) | `+ .or(PENDING_APPROVAL_OR)` |
| **Page-tab** badge | [TimeEntries.tsx](src/pages/TimeEntries.tsx) | same as sidebar; realtime sub de-scoped from `category=eq.labor_internal` so subcontractor-time changes refresh it too |

Because all three read the same column + clauses, **the badge count always equals the rows the table
can display**, and clockless PTO is now visible and approvable (the row + mobile card already render
NULL clock times as `-`).

**Common pitfalls:**
- Don't re-add `start_time`/`end_time`/`category` filters to any time-approval query — read
  `is_time_entry`. If you add a fourth surface, consume `src/utils/timeEntries.ts`, don't re-derive.
- Don't write `is_time_entry` (it's generated — Postgres errors). Change the inputs (`category` /
  `start_time`) and it follows.
- Keep `EXCLUDE_ACTIVE_TIMERS_OR` paired with `.eq('is_time_entry', true)` — without it, in-progress
  timers (start_time set, end_time NULL → `is_time_entry=true`) leak back into the approval queue as
  phantom 0h/$0 rows.
- Don't add `hours > 0` as a labor discriminator — historical PTO can have NULL hours; it would strand
  exactly the rows this fix rescued.
...
68. **Time-approval badge counted rows the table couldn't display — three surfaces, three different "what is a time entry" filters (Jun 3, 2026, PR [#143](https://github.com/Cradcliff187/profitbuild-dash/pull/143))** — User reported the Time Approvals badge stuck on a count (3) with the table empty when filtered to pending. Root cause was definition drift: the table ([useTimeEntries.ts](src/hooks/useTimeEntries.ts)) required `start_time AND end_time NOT NULL`, the sidebar badge ([usePendingCounts.ts](src/hooks/usePendingCounts.ts)) used `start_time NOT NULL`, and the page-tab badge ([TimeEntries.tsx](src/pages/TimeEntries.tsx)) used `category='labor_internal'` — three filters that disagree. Manually-entered PTO (Sick/Vacation/Holiday) has hours but **NO clock times**, so it was counted by the `labor_internal` badge yet structurally invisible in the table (which demanded clock times), and there was **no in-app way to approve it**. On the same production data the three surfaces literally showed `1 / 3 / 1` for the same concept. **Investigation note worth remembering**: the live counts shifted between queries because an admin was approving entries in the app during diagnosis — re-pull current state rather than trusting a count from minutes earlier. Also: `hours > 0` looked like a clean discriminator (773 labor rows have hours, 0 money expenses do) until 4 approved Vacation rows turned up with **NULL hours** — proving PTO can't be identified by hours either. **Fix** = declare it in the DB: the `expenses.is_time_entry` generated column (Architectural Rule 33) as the single source of truth, consumed by all three surfaces via [src/utils/timeEntries.ts](src/utils/timeEntries.ts), so badge == table forever. Also centralized `PTO_PROJECT_NUMBERS` (was duplicated in 6 files). **Diagnostic pattern**: "a badge count with no matching rows" = the counter and the list are using different predicates against the same table; find every place that re-implements the predicate and converge them on one shared definition (ideally a DB column), don't just patch the one surface that's visibly wrong.
...
| Issue | Resolution |
|-------|-----------|
| **Time-approval badge counted PTO rows the table couldn't show (PR [#143](https://github.com/Cradcliff187/profitbuild-dash/pull/143), Jun 3, 2026)** | Fixed at the root — see Architectural Rule 33 + Gotcha #68. The approval table and the two badges (sidebar + page-tab) each used a different filter for "a pending time entry" (`start_time AND end_time` / `start_time` / `category='labor_internal'`), so on the same data they showed `1 / 3 / 1`. Manually-entered PTO (Sick/Vacation/Holiday, no clock times) was counted by the `labor_internal` badge but invisible in the table — uncountable AND unapprovable. **Fix**: new `expenses.is_time_entry` GENERATED STORED column (`category='labor_internal' OR start_time IS NOT NULL`) applied to production via MCP migration `20260603140407_add_is_time_entry_generated_column` (placeholder file committed; types regenerated) — self-maintaining, no backfill/trigger. All three surfaces now read it via the shared [src/utils/timeEntries.ts](src/utils/timeEntries.ts) (`is_time_entry` + `EXCLUDE_ACTIVE_TIMERS_OR` + `PENDING_APPROVAL_OR`), so badge == table and clockless PTO is visible/approvable; active timers stay excluded. Also centralized `PTO_PROJECT_NUMBERS` (was duplicated in 6 files). Verified on prod: 777 labor rows flagged (758 internal + 19 subcontractor time, 0 money), badge==table==4 with all three PTO types present, 0 active timers leaking; type-check clean. **Requires Lovable Publish** to go live. |
