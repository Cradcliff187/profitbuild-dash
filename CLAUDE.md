...
| Issue | Resolution |
|-------|-----------|
| **`/estimates` (sidebar) crashed to ErrorBoundary fallback after saving a new estimate — `Cannot read properties of undefined (reading 'split')` (PR [#95](https://github.com/Cradcliff187/profitbuild-dash/pull/95), May 20 2026)** | Fixed — console stack pointed at `extractProjectCounter` ([src/utils/numberGeneration.ts:46](src/utils/numberGeneration.ts:46)) called from [EstimatesTableView.tsx:175](src/components/EstimatesTableView.tsx:175) grouping map. Root cause: [Estimates.handleSaveEstimate](src/pages/Estimates.tsx:360) optimistically appends the estimate object returned by `EstimateForm` (built from form state — no joined `project_number` / `project_name` / `client_name`), then fires `loadEstimates()` async. React re-renders synchronously with the incomplete row first; `extractProjectCounter(projectEstimates[0].project_number)` calls `.split('-')` on undefined and throws. Verified at the DB level the join works fine for SYS-TEST — it's purely the optimistic-render frame before `loadEstimates()` resolves. Fix hardens the read side rather than the optimistic write: (a) `extractProjectCounter` now accepts `string \| null \| undefined` and returns `0` for falsy/NaN inputs; (b) [EstimatesTableView](src/components/EstimatesTableView.tsx) and [QuotesTableView](src/components/QuotesTableView.tsx) fall back to placeholder strings (`—` / "Untitled Project") when the join fields are missing. Once `loadEstimates()` resolves the row repaints with real values; placeholders only show for the optimistic-render frame. `QuotesTableView` got the same defensive treatment proactively (identical `extractProjectCounter` + `projectQuotes[0].*` group label pattern). **Deeper fix deferred**: dropping the optimistic update in `handleSaveEstimate` or threading joined fields through `EstimateForm.onSave` — both touch the save flow and have wider blast radius than today's user-visible bug warrants. **Lesson**: when an optimistic update appends a partial row, audit downstream renderers for non-null assumptions on fields the optimistic row won't carry; harden the renderer rather than the appender unless you also want to rewrite the save flow. |
| **Estimate save toasts stacked instead of replacing each other — "Processing estimate details..." sat on screen next to the final "First Estimate Created" / "Estimate Updated" toast (PR [#94](https://github.com/Cradcliff187/profitbuild-dash/pull/94), May 20 2026)** | Fixed — [EstimateForm.handleSubmit](src/components/EstimateForm.tsx) fired an optimistic `toast.info(...)` at the top and a separate `toast.success/error(...)` at the end; with no shared id sonner stacked them. Switched the optimistic toast to `toast.loading()` and captured its id, then passed `{ id: toastId }` to every terminal toast in the handler (success on insert, version bump, update; error on project-fetch failure; catch-block error). Sonner now updates the existing card in place from loading → success/error instead of stacking a second. Five terminal toasts wired in: the project-fetch error (L606), the three version/update success toasts (L725, L855, L986), the first-estimate success (L1109), and the catch-block error (L1125). |
