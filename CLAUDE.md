...
**Admin-only exception (resolved Apr 16, 2026)**: `BulkExpenseAllocationSheet` now applies server-side `.eq('is_split', false)` + `.range(0, 9999)` + `.order('expense_date', desc)` to its candidates query. The correlations dedup stays client-side (pushing it server-side via `.not('id', 'in', ...)` risks URL-length limits at 500+ correlated UUIDs). See resolved items below.
...
| **Legacy `NoteInput` component still live on desktop `ProjectNotesTimeline`** | `src/components/notes/NoteInput.tsx`, consumed by `ProjectNotesTimeline.tsx` (desktop resizable-panel layout) | Rule 15's `NoteComposer` unification covered mobile surfaces only. Desktop timeline still uses `NoteInput` with its Photo/Video/File 3-button row + `voiceNoteSlot` extension-point pattern. Future work: evaluate whether `<NoteComposer presentation="inline" />` is a drop-in — likely yes, but the desktop panel has subtly different layout constraints (fixed-height scroll context, resizable splits). Not urgent; the mobile surface is where users composed and where drift was happening. |
| **Dead code: `NotesSheetTrigger`** | `src/components/schedule/NotesSheetTrigger.tsx` | Zero callers in the codebase. Opens a full-screen sheet with `ProjectNotesTimeline`. Safe to delete. Flagged during NoteComposer unification exploration but left for a dedicated cleanup pass to avoid diff noise in the refactor commits. |
