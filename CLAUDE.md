...
| **`BulkExpenseAllocationSheet` silently truncates candidates at 1,000 rows** | [src/components/BulkExpenseAllocationSheet.tsx:87](src/components/BulkExpenseAllocationSheet.tsx:87) | The Apr 16 fix added `.eq('is_split', false)` + `.order(...)` + `.range(0, 9999)`. The is_split and ordering parts are correct; the `.range(0, 9999)` was expected to raise the cap but this project has `db-max-rows = 1000` server-side (see Gotcha #23 correction) so it caps at 1,000. At 1,288 non-split expenses today the admin bulk-allocate modal is missing ~288 candidate rows silently. **Proper fix:** switch the candidates query to loop-paginated fetch (the pattern `ExpenseExportModal` now uses) OR push to a server-side RPC that returns only un-correlated expenses. |
