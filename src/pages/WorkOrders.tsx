...
const WorkOrders = () => {
  ...
  const loadWorkOrders = async () => {
    try {
      ...
      const processedData = data?.map(project => {
        ...
        const revenues = project.project_revenues || [];
        ...
        return {
          ...
          total_invoiced: revenues.reduce((sum: number, rev: any) => sum + (Number(rev.amount) || 0), 0),
        };
      }) || [];
  ...
  // Calculate Running PO Total (aggregate total_invoiced by customer_po_number)
  const poRunningTotals = useMemo(() => {
    const poMap = new Map<string, number>();
    workOrders.forEach(wo => {
      if (!wo.customer_po_number) return;
      const current = poMap.get(wo.customer_po_number) || 0;
      poMap.set(wo.customer_po_number, current + (wo.total_invoiced || 0));
    });
    return poMap;
  }, [workOrders]);
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          ...
          <WorkOrdersTableView
            ...
            poRunningTotals={poRunningTotals}
          />
        </div>
      )}
