...
const TimeEntriesPage = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(
    timeEntryColumnDefinitions.filter(col => col.required || col.key === "status" || col.key === "hours" || col.key === "gross_hours" || col.key === "notes").map(col => col.key)
  );
  ...
  return (
    <MobilePageWrapper onRefresh={refreshTimeEntries} enablePullToRefresh>
      <PageHeader
        icon={ClipboardCheck}
        title="Time Entries"
