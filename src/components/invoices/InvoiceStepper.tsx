...
export type InvoiceStep = 'configure' | 'preview' | 'complete';

interface InvoiceStepperProps {
  currentStep: InvoiceStep;
}

const STEPS: { id: InvoiceStep; label: string; number: number }[] = [
  { id: 'configure', label: 'Configure', number: 1 },
  { id: 'preview', label: 'Preview', number: 2 },
  { id: 'complete', label: 'Complete', number: 3 },
];

const STEP_ORDER: InvoiceStep[] = ['configure', 'preview', 'complete'];

export function InvoiceStepper({ currentStep }: InvoiceStepperProps) {
  const getStepStatus = (
    stepId: InvoiceStep
