...
DECLARE
  ...
  contingency_used DECIMAL(15,2) := 0;
  remaining DECIMAL(15,2) := 0;
BEGIN
  -- Get total contingency from current estimate
  SELECT COALESCE(contingency_amount, 0)
  INTO total_contingency
  FROM public.estimates
  WHERE project_id = project_id_param AND is_current_version = true
  LIMIT 1;
  
  -- Get contingency used from current estimate
  SELECT COALESCE(contingency_used, 0)
  INTO contingency_used
  FROM public.estimates
  WHERE project_id = project_id_param AND is_current_version = true
  LIMIT 1;
  
  -- Calculate remaining contingency
  remaining := total_contingency - contingency_used;
  
  RETURN GREATEST(remaining, 0); -- Never return negative
END;
$$;
...
BEGIN
  ...
  SELECT margin_percentage, minimum_margin_threshold, target_margin
...
SET 
  ...
  target_margin = 20.0,
  contingency_remaining = public.calculate_contingency_remaining(id);
