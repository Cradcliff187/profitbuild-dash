-- Fix calculate_project_margins function - correct quote cost calculation
-- Drop and recreate to fix parameter defaults issue

DROP FUNCTION IF EXISTS public.calculate_project_margins(UUID);

CREATE FUNCTION public.calculate_project_margins(project_id_param UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET
    ...
    projected_costs = projected_costs_calc,
    ...
    contingency_allocated = contingency_amt,
    updated_at = NOW()
