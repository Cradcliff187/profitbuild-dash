-- Fix enum type filtering in execute_simple_report
-- Problem: Enum fields like status, category need explicit casting when comparing with text values
-- Solution: Cast enum fields to text for comparison operations
-- Error: "operator does not exist: project_status = text"
...
DECLARE
  ...
  condition_sql TEXT;
  i INTEGER;
  -- Enum fields that need casting to text for comparison
  enum_fields TEXT[] := ARRAY['status', 'category', 'project_type', 'approval_status', 'quote_status', 'project_category'];
...
BEGIN
  ...
  IF p_filters IS NOT NULL AND p_filters != '{}'::jsonb THEN
    ...
    LOOP
      ...
      -- Check if field is an enum type that needs casting
      is_enum_field := filter_field = ANY(enum_fields);
      ...
      IF filter_operator = 'contains_any' THEN
        ...
        IF array_length(filter_array, 1) > 0 THEN
          IF is_enum_field THEN
            where_conditions := array_append(where_conditions, format('%I::text && ARRAY[%s]::TEXT[]', filter_field, (SELECT string_agg(quote_literal(v), ',') FROM unnest(filter_array) AS v)));
          ELSE
      ...
      ELSIF filter_operator = 'contains_only' THEN
        ...
        IF array_length(filter_array, 1) > 0 THEN
          IF is_enum_field THEN
            where_conditions := array_append(where_conditions, format('%I::text <@ ARRAY[%s]::TEXT[] AND NOT (%I::text && ARRAY[]::TEXT[])', filter_field, (SELECT string_agg(quote_literal(v), ',') FROM unnest(filter_array) AS v), filter_field));
          ELSE
      ...
      ELSIF filter_operator = 'equals' THEN
        IF is_enum_field THEN
          -- Cast enum field to text for comparison
          where_conditions := array_append(where_conditions, format('%I::text = %L', filter_field, filter_value->>'value'));
        ELSE
      ...
      ELSIF filter_operator = 'contains' THEN
        where_conditions := array_append(where_conditions, format('%I::text ILIKE %L', filter_field, '%' || (filter_value->>'value') || '%'));
      ...
      ELSIF filter_operator = 'in' THEN
        ...
        IF array_length(filter_array, 1) > 0 THEN
          IF is_enum_field THEN
            -- Cast enum field to text for ANY comparison
            where_conditions := array_append(where_conditions, format('%I::text = ANY(ARRAY[%s]::TEXT[])', filter_field, (SELECT string_agg(quote_literal(v), ',') FROM unnest(filter_array) AS v)));
          ELSE
            where_conditions := array_append(where_conditions, format('%I = ANY(ARRAY[%s]::TEXT[])', filter_field, (SELECT string_agg(quote_literal(v), ',') FROM unnest(filter_array) AS v)));
          END IF;
        END IF;
