-- Fix calculate_project_margins function to eliminate double-counting and use correct cost basis
...
BEGIN
  ...
  -- Get accepted quotes total (for tracking only, not for margin calculations)
  ...
  -- FIX #1: Calculate contracted amount WITHOUT double-counting contingency
  -- The change_order_client_total already includes contingency_billed_to_client
  ...
  -- FIX #2: Calculate adjusted costs using ESTIMATE costs (not quote costs)
  -- Adjusted costs = original estimate costs + change order cost impacts
...
-- Recalculate margins for all projects to apply the fixes
DO $$
DECLARE
