-- Comprehensive fix for project financial calculations

-- 1. Fix calculate_project_margins() to properly handle contingency billing
...
BEGIN
  ...
  -- Get change order totals AND contingency billed amount
  SELECT 
    COALESCE(SUM(co.client_amount), 0),
    COALESCE(SUM(co.cost_impact), 0),
    COALESCE(SUM(co.margin_impact), 0),
    COALESCE(SUM(
      CASE WHEN co.includes_contingency THEN 
        (SELECT e.contingency_amount 
         FROM public.estimates e 
         WHERE e.project_id = co.project_id 
...
-- 2. Fix calculate_contingency_remaining() to return 0 when contingency is billed
...
DECLARE
  ...
  contingency_billed BOOLEAN := false;
  remaining_contingency DECIMAL(15,2) := 0;
BEGIN
  -- Get total contingency from approved estimate
  ...
  -- Check if contingency has been billed via change order
  SELECT EXISTS (
    SELECT 1 FROM public.change_orders co
    WHERE co.project_id = project_id_param 
      AND co.status = 'approved' 
      AND co.includes_contingency = true
  ) INTO contingency_billed;
  
  -- If contingency was billed, it's fully consumed
  IF contingency_billed THEN
    RETURN 0;
  END IF;
...
-- 3. Fix trigger to fire on field changes and remove manual contracted_amount update
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'change_orders' THEN
    -- Trigger when change order is approved OR when key fields change on approved CO
    IF (NEW.status = 'approved' AND (OLD.status IS DISTINCT FROM NEW.status))
       OR 
       (NEW.status = 'approved' AND OLD.status = 'approved' AND 
        (OLD.includes_contingency IS DISTINCT FROM NEW.includes_contingency OR
         OLD.client_amount IS DISTINCT FROM NEW.client_amount OR
         OLD.cost_impact IS DISTINCT FROM NEW.cost_impact))
    THEN
      -- Let calculate_project_margins handle all calculations including contracted_amount
      PERFORM public.calculate_project_margins(NEW.project_id);
    END IF;
  END IF;
...
-- 4. Recalculate margins for project 225-007 with corrected logic
