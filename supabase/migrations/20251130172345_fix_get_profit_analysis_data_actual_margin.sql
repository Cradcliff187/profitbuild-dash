-- Fix get_profit_analysis_data function to include actual_margin
-- The function in the database was missing actual_margin in RETURNS TABLE and SELECT
-- Must drop and recreate because return type is changing

DROP FUNCTION IF EXISTS get_profit_analysis_data(text[]);

CREATE FUNCTION get_profit_analysis_data(
...
AS $$
  SELECT 
    ...
    COALESCE(actual_margin, 0),
    -- Margin percentage based on contracted amount (for projected margin)
    CASE 
      WHEN COALESCE(contracted_amount, 0) > 0 
      THEN (COALESCE(projected_margin, 0) / contracted_amount) * 100
      ELSE 0
