-- Fix naming conflict in calculate_project_margins function
...
DECLARE
  ...
  projected_cost_total DECIMAL(15,2) := 0;
...
BEGIN
  ...
  INTO original_cost_total
  FROM public.estimate_line_items eli
  JOIN public.estimates e ON eli.estimate_id = e.id
  WHERE e.project_id = project_id_param AND e.status = 'approved';
  
  original_margin_calc := approved_estimate_total - original_cost_total;
  ...
  WITH line_item_costs AS (
    SELECT 
      ...
      ) as line_projected_cost
    FROM public.estimate_line_items eli
    JOIN public.estimates e ON eli.estimate_id = e.id
    WHERE e.project_id = project_id_param AND e.status = 'approved'
  )
  SELECT COALESCE(SUM(line_projected_cost), 0)
  INTO projected_cost_total
  FROM line_item_costs;
  
  projected_margin_calc := approved_estimate_total - projected_cost_total;
