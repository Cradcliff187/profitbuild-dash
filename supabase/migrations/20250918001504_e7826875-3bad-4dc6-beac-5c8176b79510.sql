-- Add profit tracking columns to change_orders table
ALTER TABLE public.change_orders 
ADD COLUMN client_amount DECIMAL(15,2),
ADD COLUMN cost_impact DECIMAL(15,2),
ADD COLUMN margin_impact DECIMAL(15,2),
ADD COLUMN includes_contingency BOOLEAN DEFAULT false;

-- Create function to calculate change order margin impact
...
-- Create trigger for automatic margin impact calculation
CREATE TRIGGER calculate_change_order_margin_impact_trigger
  BEFORE INSERT OR UPDATE ON public.change_orders
  FOR EACH ROW
  EXECUTE FUNCTION public.calculate_change_order_margin_impact();

-- Update the calculate_project_margins function to include change order impacts
...
-- Update calculate_contingency_remaining to account for change orders using contingency
